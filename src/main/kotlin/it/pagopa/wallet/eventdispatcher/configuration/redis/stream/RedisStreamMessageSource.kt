    ...
    init {
        runCatching {
            ...
            .onFailure {
                ...
                if (redisStreamConf.failOnErrorCreatingConsumerGroup) {
                    throw IllegalStateException(
                        "Error creating consumer group for stream event receiver",
                        it
                    )
                }
            }
                    ...
                    redisStreamReceiver
                        ...
                        .map {
                            RedisStreamMessageSourceLogger.logger.debug(
                                "Redis stream deserialization map: {}",
                                it.value
                            )
                            val parsedEvent =
                ...
                .onFailure {
                    ...
                    if (it !is IllegalStateException) {
                        RedisStreamMessageSourceLogger.logger.error(
                            "Exception receiving Redis Stream event",
                            it
                        )
                    }
                }
